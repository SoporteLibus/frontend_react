...
export default function LoadFile() {
    ...
    const readFile = (e) => {
        ...
        for (let i = 0; i<100; i++){
            ...
            Myelement1[i]=JSON.stringify({'numero_de_operacion':extraccion[i][0],
            ...
            const post = async () => {
                try {
                    const data={'numero_de_operacion':extraccion[i][0],
                    'codigo_de_producto':extraccion[i][1],
                    'materia_prima':extraccion[i][9],
                    'numero_de_inyectora':extraccion[i][7],
                    'numero_de_molde':extraccion[i][3],
...
return (
    ...
    <Form title="Carga de datos" action="#" method="POST" name="formFormulario" 
        divgroup="div-group-block" nameButton="Recargar" classButton="display-none" >
        <input type="file" onChange={ readFile } />
        <br />
        <div>
        <p>Info en formato JSON: <br />
        <textarea name="comentario" value={ text } 
        onChange={(e) => setText(e.target.value)} cols={136} rows={30} />
        <br />
        </p>
        </div>
    </Form>
